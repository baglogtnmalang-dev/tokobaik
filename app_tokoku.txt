...
# Gunakan DATABASE_URL dari environment variable jika ada (untuk hosting),
# jika tidak ada, gunakan SQLite lokal (untuk development).
database_url = os.environ.get('DATABASE_URL')
# Jika database_url ada dan diawali dengan 'postgres://', ubah menjadi 'postgresql://'
# Ini diperlukan untuk kompatibilitas dengan SQLAlchemy versi baru di lingkungan hosting (misalnya Heroku/Render)
...
if __name__ == '__main__':
    # Jika menggunakan SQLite, inisiasi database lokal
    if toko_app.config['SQLALCHEMY_DATABASE_URI'].startswith('sqlite'):
    # Memastikan database diinisiasi saat file dijalankan
    init_db() 
    else:
        # Jika menggunakan database eksternal, kita hanya memastikan tabelnya ada.
        # Catatan: Di lingkungan hosting, Anda mungkin perlu menjalankan 'flask db upgrade' secara manual 
        # jika Anda menggunakan Flask-Migrate, atau menggunakan Build Command Render.
        with toko_app.app_context():
            db.create_all()
